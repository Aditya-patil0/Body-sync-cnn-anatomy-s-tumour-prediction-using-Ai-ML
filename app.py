...
MODEL_PATHS = {
    "model1": "models/brain_tumor_model.h5",
...
brain = load_model(MODEL_PATHS["model1"])
breast = load_model(MODEL_PATHS["model2"])
kidney = load_model(MODEL_PATHS["model3"])
lung = load_model(MODEL_PATHS["model4"])
oral = load_model(MODEL_PATHS["model5"])
...
def analyze_tumor():
    ...
    try:
        ...
        if selected_category == "Brain Tumor":
            ...
            prediction = predict_tumor(brain, preprocessed_img)
        elif selected_category == "Breast Tumor":
            preprocessed_img = preprocess_image(image_path)
            prediction = predict_tumor(breast, preprocessed_img)
        elif selected_category == "Kidney Tumor":
            preprocessed_img = preprocess_image(image_path)
            prediction = predict_tumor(kidney, preprocessed_img)
        elif selected_category == "Lung Tumor":
            preprocessed_img = preprocess_image(image_path)
            prediction = predict_tumor(lung, preprocessed_img)
        elif selected_category == "Oral Tumor":
            preprocessed_img = preprocess_image(image_path)
            prediction = predict_tumor(oral,preprocessed_img)
        else:
